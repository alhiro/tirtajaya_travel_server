const service = require("../services/go_send");
const sequelizeTransaction = require('../config/sequelizeTransaction')

exports.get = async (req, res, next) => {
  const id = req.query.go_send_id;
  try {
...
exports.list = async (req, res, next) => {
  const params = { page: req.query.page, limit: req.query.limit, search: req.query.search };
  try {
        var all = await service.list(params);
        return res.status(200).json(
          { 
            success: all.success,
            data: all.data,
...
exports.searchByFilter = async function (req, res, next) {
  ...
  try {
        ...
        return res.status(200).json({ status: 200, data: partners, message: "Search data success" });
  } catch (err) {
    return res
      .status(500)
      .send({ code: 500, success: false, message: err.message, data: { err } });
