...
exports.updateObject = async function(req, res, next) {
  try {
    let update = await service.updateObject(req);
    return res.status(200).json(
      {
        status: update.status,
        data: update.data,
...
exports.searchByFilter = async function (req, res, next) {
  ...
  try {
        ...
        return res.status(200).json({ status: 200, data: partners, message: "Search address package success" });
  } catch (err) {
    return res
      .status(500)
      .send({ code: 500, success: false, message: err.message, data: { err } });
