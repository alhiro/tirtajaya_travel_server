const service = require("../services/delivery_package");
const sequelizeTransaction = require('../config/sequelizeTransaction')

exports.get = async (req, res, next) => {
  const id = req.query.delivery_package_id;
  try {
...
exports.update = async function(req, res, next) {
  console.log("req :", req)
  const id = req.delivery_package_id;
...
exports.searchByFilter = async function (req, res, next) {
  ...
  try {
        ...
        return res.status(200).json({ status: 200, data: partners, message: "Search delivery package success" });
  } catch (err) {
    return res
      .status(500)
      .send({ code: 500, success: false, message: err.message, data: { err } });
