const service = require("../services/category_sub");
const sequelizeTransaction = require('../config/sequelizeTransaction')

exports.get = async (req, res, next) => {
  const id = req.query.category_sub_id;
  try {
...
exports.searchByFilter = async function (req, res, next) {
  ...
  try {
    ...
    return res.status(200).json({ status: 200, data: partners, message: "Search category sub package success" });
  } catch (err) {
    return res
      .status(500)
      .send({ code: 500, success: false, message: err.message, data: { err } });
